...
import {
  ...
  alpha, Menu, MenuItem, Button, Tooltip as MuiTooltip,
} from "@mui/material";
...
const TruncatedCell = ({ value, maxWidth = 160, mono = false, weight = 500, color = "#475569" }) => (
  <MuiTooltip title={value || "—"} enterDelay={400}>
    <Typography
      variant="body2"
      noWrap
      sx={{
        maxWidth, color, fontWeight: weight,
 ...
 *    alongside the existing "Exceptions" count.
 *
...
const PoWiseExceptionsTable = ({
  ...
  // (i.e. on the standalone PO Data page) — the dashboard's call site never
  // passes showTotals, so its column set is exactly what it was before.
  const scrollColumns = useMemo(() => {
    const cols = [
      { key: "lineItemsDisplay", label: "Line Item(s)", sortKey: "distinctLineItems", minWidth: 130 },
    ...
    cols.push({ key: "exceptionLineCount", label: "Exceptions", minWidth: 110 });
    if (showTotals) {
  ...
  // whether or not the extra showTotals columns are present.
  const renderCell = (col, r) => {
    switch (col.key) {
      ...
      case "purchase_req":
        return <TruncatedCell value={r.purchase_req} maxWidth={110} weight={500} />;
      case "vendorName":
        return <TruncatedCell value={r.vendorName || r.vendorCode} maxWidth={175} weight={600} color="#0f172a" />;
      case "vendorGstin":
        return <TruncatedCell value={r.vendorGstin} maxWidth={135} mono />;
      case "plantName":
        return <TruncatedCell value={r.plantName || r.plant} maxWidth={135} />;
      case "poTypeName":
        return <TruncatedCell value={r.poTypeName || r.poType} maxWidth={135} />;
      case "taxCode":
        return <TruncatedCell value={r.taxCode} maxWidth={85} mono />;
      case "purchaseGroupName":
        return <TruncatedCell value={r.purchaseGroupName || r.purchaseGroup} maxWidth={155} />;
      case "paymentTermDescription":
        return <TruncatedCell value={r.paymentTermDescription || r.paymentTerm} maxWidth={155} />;
      case "totalLineCount":
        return (
          <Typography variant="body2" sx={{ fontWeight: 600, color: "#0f172a", fontVariantNumeric: "tabular-nums" }}>
            {r.totalLineCount ?? "—"}
          </Typography>
        );
      case "exceptionLineCount":
        return (
          <Chip
            ...
            sx={{
              height: 24, minWidth: 32, fontWeight: 700, fontSize: "0.75rem",
              bgcolor: r.exceptionLineCount > 0 ? alpha("#dc2626", 0.1) : alpha("#059669", 0.1),
        ...
        return (
          <Chip
            ...
            sx={{
              ...
              bgcolor:
                r.compliancePct == null ? "grey.100"
                : r.compliancePct >= 80 ? alpha("#059669", 0.1)
                : r.compliancePct >= 50 ? alpha("#d97706", 0.1)
                : alpha("#dc2626", 0.1),
              color:
                r.compliancePct == null ? "text.secondary"
                : r.compliancePct >= 80 ? "#059669"
                : r.compliancePct >= 50 ? "#d97706"
                : "#dc2626",
            }}
          />
        );
      case "valueExposure":
        return typeof r.valueExposure === "number"
          ? r.valueExposure.toLocaleString(undefined, { maximumFractionDigits: 0 })
          : (r.valueExposure ?? "—");
      default:
        return r[col.key] ?? "—";
    }
  ...
  const headerCellSx = {
    height: HEADER_HEIGHT, bgcolor: "#f8fafc", fontWeight: 700, color: "#475569",
    borderBottom: "2px solid", borderColor: "grey.100", whiteSpace: "nowrap",
    py: 0,
  };

  const rowSx = {
    height: ROW_HEIGHT, cursor: "pointer", "&:last-child td": { border: 0 },
    ...
    <Paper elevation={0} sx={{
      ...
      <Box sx={{ p: 3, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
        <Box>
          <Typography variant="h6" sx={{ fontWeight: 800, color: "#0f172a", display: "flex", alignItems: "center" }}>
            {title}
            <Chip size="small" label={`${rows?.length ?? 0} POs`} sx={{ ml: 2, fontWeight: 700, bgcolor: alpha("#4f46e5", 0.1), color: "#4f46e5" }} />
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
          ...
          {viewAllHref && (
            <Button
              ...
              sx={{ borderRadius: 2, fontWeight: 700, textTransform: "none", bgcolor: alpha("#4f46e5", 0.08), color: "#4f46e5", "&:hover": { bgcolor: alpha("#4f46e5", 0.16) } }}
            >
              Open PO Data page
            </Button>
          )}
      ...
      {loading ? (
        ...
        <Box sx={{ display: "flex", position: "relative" }}>
          ...
          <Box
            sx={{
              flexShrink: 0, width: FROZEN_COL_WIDTH, bgcolor: "#fff",
              borderRight: "1px solid", borderColor: "grey.200",
              boxShadow: "3px 0 6px -4px rgba(0,0,0,0.12)", zIndex: 2, position: "relative",
            }}
          >
            <Box
              ref={frozenScrollRef}
              ...
              sx={{
                maxHeight: BODY_MAX_HEIGHT, overflowY: "auto", overflowX: "hidden",
            ...
            >
              <Table sx={{ width: FROZEN_COL_WIDTH, tableLayout: "fixed" }}>
                ...
                <TableBody>
                  {filtered.map((r) => (
                    <TableRow key={r.poNumber} hover sx={rowSx} onClick={(e) => openRowMenu(e, r)}>
                      <TableCell sx={{ fontWeight: 600, color: "#0f172a", whiteSpace: "nowrap", height: ROW_HEIGHT }}>
                        {r.poNumber}
                      </TableCell>
                    </TableRow>
                  ))}
                  {filtered.length === 0 && (
                    <TableRow sx={{ height: ROW_HEIGHT }}><TableCell /></TableRow>
                  )}
                </TableBody>
              </Table>
            </Box>
          ...
          <Box
            ...
            sx={{ flex: 1, maxHeight: BODY_MAX_HEIGHT, overflow: "auto", minWidth: 0 }}
          >
            <Table size="medium" sx={{ minWidth: showTotals ? 1740 : 1500 }}>
              <TableHead>
                <TableRow>
                  {scrollColumns.map((c) => (
                    <TableCell
                      ...
                      sx={{ ...headerCellSx, minWidth: c.minWidth, ...(RIGHT_ALIGN_KEYS.has(c.key) && { pr: 3 }) }}
                    >
                      <TableSortLabel
              ...
              <TableBody>
                  ...
                  <TableRow key={r.poNumber} hover sx={rowSx} onClick={(e) => openRowMenu(e, r)}>
                    {scrollColumns.map((c) => (
                      <TableCell
                        ...
                        sx={{
                          whiteSpace: "nowrap",
                          ...(RIGHT_ALIGN_KEYS.has(c.key) && { pr: 3, fontVariantNumeric: "tabular-nums" }),
                          ...(c.key === "valueExposure" && { fontWeight: 600, color: "#0f172a" }),
                        }}
                      >
                        {renderCell(c, r)}
                      </TableCell>
                ...
                {filtered.length === 0 && (
                  <TableRow sx={{ height: ROW_HEIGHT }}>
                    <TableCell colSpan={scrollColumns.length} align="center" sx={{ color: "text.secondary" }}>
      ...
      <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={closeRowMenu} elevation={3} sx={{ "& .MuiPaper-root": { borderRadius: 2, mt: 0.5 } }}>
        <MenuItem
          onClick={() => {
        ...
        >
          ...
          <Typography variant="body2" sx={{ fontWeight: 500 }}>Open in New Tab</Typography>
        </MenuItem>
        <MenuItem
          onClick={() => {
        ...
        >
          ...
          <Typography variant="body2" sx={{ fontWeight: 500 }}>View Details Here</Typography>
        </MenuItem>
      </Menu>
    </Paper>
  );
