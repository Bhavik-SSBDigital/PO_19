        ...
        {loading || !result ? (
          ...
          <Box>
            <Grid container spacing={2}>
              ...
              <Field label="Vendor" value={result.nameOfVendor || result.vendor_code} />
              <Field label="Material" value={`${result.material_code || ""} ${result.material_disc || ""}`} />
              <Field label="Plant" value={result.plant} />
              ...
              />
              <Field label="Payment Term" value={result.payment_term} />
            ...
            <List dense>
              {(result.results || []).map((point) => (
                <ListItem key={point.pointNo} disableGutters>
                  <ListItemText
                    primary={`Point ${point.pointNo}: ${
                      point.verified ? "Verified" : point.not_applicable ? "Not Applicable" : "Not Verified"
                    }`}
                    secondary={(point.remarks || []).join("; ")}
                  />
                </ListItem>
              ))}
            </List>
