...
import BPVTable from "./components/bpv-table";
...
const tabNumber = { closed: "1", assigned: "2", unassigned: "3" };
...
totalPages = Math.ceil(response.total / response.pageSize);
  ...
  const handleChangeMultiPO = (e) => {
    ...
    parts.forEach((num) => {
      if (isTenDigitNumber(num) && !PONumbers.includes(num)) {
        valid.push(num);
      }
    });
  ...
  const handleChange2 = (_, newValue) => {
    ...
    setInputs((prevInputs) => ({ ...prevInputs, page: 1, manual: newValue }));
  };

  const handleChangePage = (_, newPage) => {
    setInputs((prevInputs) => ({ ...prevInputs, page: newPage }));
    getPageData({
      ...inputs,
      page: newPage,
      documentNumbers,
    ...
    <>
          ...
          {!loading && (
            <TabContext value={inputs.auditType}>
              ...
              >
                ...
                <Tab label="Assigned Audits" value={"assigned"} />
                <Tab label="Unassigned Audits" value={"unassigned"} />
              </TabList>
            </TabContext>
          )}
        </Box>
      ...
      {(dataViewType === "PJV" || dataViewType === "NONPO") && (
        <InvoiceTable
          ...
          loading={loading}
          value={tabNumber[inputs.auditType]}
        />
      )}
      {dataViewType === "PO" && (
        <POTable
